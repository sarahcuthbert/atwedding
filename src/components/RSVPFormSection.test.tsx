...
test('displays rsvp form for invitee with input', () => {
    ...
    render(<RSVPFormSection invitee={invitee} index={0} attending={true} setAttending={vi.fn()} dietary={""}
                            setDietary={vi.fn()} other={""} setOther={vi.fn()}/>)

    expect(screen.getByRole("heading", {name: "RSVP details for John Smith"})).toBeDefined();
    expect(screen.getByRole("radiogroup", {name: "Attending?"})).toBeDefined();
    expect(screen.getByRole("textbox", {name: "Preferences and Allergens"})).toBeDefined();
...
test('calls set methods on input change', async () => {
    ...
    render(<RSVPFormSection invitee={invitee} index={0} attending={true} setAttending={mockSetAttending} dietary={""}
                            setDietary={mockSetDietary} other={""} setOther={mockSetOther}/>)

    await userEvent.click(screen.getByRole("radio", {name: "No"}));
    expect(mockSetAttending).toHaveBeenCalledWith(false, 0);

    await userEvent.type(screen.getByRole("textbox", {name: "Preferences and Allergens"}), "veg");
