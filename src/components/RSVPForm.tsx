...
export const RSVPForm = ({ invitees, onSubmitRSVP }: RSVPFormProps) => {
    const [attendings, setAttendings] = useState<boolean[]>(
        new Array(invitees.length).fill(true)
    );
    const [dietarys, setDietarys] = useState<string[]>(
        new Array(invitees.length).fill('')
    );
