...
describe('RSVPForm', () => {
    ...
    test('should display multiple rsvp for multiple invitee', () => {
        ...
        render(
            <RSVPForm invitees={[invitee, invitee2]} onSubmitRSVP={vi.fn()} />
        );
        ...
        expect(
            screen.getByRole('heading', { name: 'RSVP details for Mary Smith' })
        ).toBeDefined();
    });
    ...
    test('should show form and error on failed submit', async () => {
        ...
        expect(
            screen.getByText('Error submitting RSVP. Please try again!')
        ).toBeDefined();
        expect(
            screen.getByRole('heading', { name: 'RSVP details for John Smith' })
        ).toBeDefined();
