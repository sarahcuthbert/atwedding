...
describe('RSVPForm', () => {
    ...
    test('displays rsvp form if onSubmitForm returns invitees', async () => {
        ...
        expect(fetchSpy).toHaveBeenCalledWith(`api/invitees?email=${invitee.email}`)
        expect(await screen.findByRole("heading", {name: "RSVP details for John Smith"})).toBeDefined();
        expect(screen.queryByRole('heading', {name: 'Please enter your email address'})).toBeNull();
